...
function fixBubbles(selector){
 ...
 function showDetailRecord(d) {
  ...
  function groupBubbles() {
    ...
    d3.selectAll('.teamDetails')
        .style("opacity",0.0)
        .style("top", function(d){return 1800 +"px";})
        .style("left",function(d){return width +"px";});
...
// hiding previous selections
  ...
  function selectBubble(){
    ...
    bubbleDivTeam
              .style("top", function(d){return height +"px";})
              .style("left",function(d){return width +"px";})
              .attr("width",0)
              .attr("height",0)
              .style("opacity",0.0);

    var top = nodeOneBubbleCenter["selected"].y+250;
    var left = nodeOneBubbleCenter["selected"].x-200;
    
    d3.selectAll('.teamDetails')
      .style("top", function(d){return top +"px";})
      .style("left",function(d){return left +"px";})
      .style("opacity",1.0)
      .html("Current Record Wins : 60  losses : 30<br> Average Defence on Wins : 89.9  on losses: 108.9"
                +"<br>Defence per quarter on wins / on losses<br>"
                + "<svg id=\"innerBar\"  width=\"380\" height=\"300\"></svg>"
                +"<br>When Away"
                + "<svg id=\"innerBarAway\"  width=\"380\" height=\"300\"></svg>"
                +"<br><br>When Home"
                + "<svg id=\"innerBarHome\"  width=\"380\" height=\"300\"></svg>"
                +"</div>");
         
    barJs.drawBar("#innerBar");
    barJs.drawBar("#innerBarAway");
    barJs.drawBar("#innerBarHome");
